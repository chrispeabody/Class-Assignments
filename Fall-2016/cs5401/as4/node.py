...
class Node:
	...
	def __eq__(self, other):
		if self.STATE['player'] == other.STATE['player'] and self.STATE['crates'] == other.STATE['crates']:
			return True
		else:
			return False
	...
	def isGoal(self):
		...
		for target in self.STATE['targets']:
			if target not in self.STATE['crates']:
				print("No. H: {}, G: {}, F: {}".format(self.H, len(self.PATH), self.F))
				return False
		# Otherwise all targets are covered
		print("Yes. H: {}, G: {}, F: {}".format(self.H, len(self.PATH), self.F))
		return True
